...
/// SEQUENTIAL
pub fn mgs_min_seq(A: &mut Vec<Vec<f64>>) -> (Vec<Vec<f64>>, Vec<Vec<i32>>) {
    // Matrix sizing
    let n = A[0].len();
...
/// PARALLEL
pub fn mgs_min_par(A: &mut Vec<Vec<f64>>) -> (Vec<Vec<f64>>, Vec<Vec<i32>>) {
    // Matrix sizing
    let n = A[0].len();
    ...
    for i in 0..n {
        ...
        // Computing dot products in parallel
